...
require 'pp'
module Jekyll
	# Need to overwrite the inspect method here because
	# the original uses < > to encapsulate the pseudo post/page objects
	# in which case the output is taken for HTML tag and
	# hidden from view.
	#
	class Post
		def inspect
	...
	class Page
		def inspect
			"#Jekyll:Page @id=#{self.name.inspect}"
		end
	end # Page
